    ...
    >
      ...
      <div className="space-y-1 max-h-60 overflow-y-auto py-1 mb-2">
        {tables.map(table => (
          ...
          >
            <div className="font-medium text-gray-800 dark:text-gray-200">{table.name}</div>
            {table.description && (
              <div className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">{table.description}</div>
            )}
          </div>
        ))}
      </div>
...
const FullCalendar7Days: React.FC<CalendarProps> = ({
  reservations,
  tables,
  onSelectSlot,
  onCancelReservation
  ...
  // Función para manejar la cancelación de reserva  const handleEventClick = async (clickInfo: EventClickArg) => {
  ...
  const renderEventContent = (eventInfo: EventContentArg) => {
    ...
    const startTime = formatHour(event.start.toISOString());
    const endTime = formatHour(event.end.toISOString());
  ...
  return (
    <div className="h-[600px] relative" ref={(el) => el}>
      <div className="text-center mb-2 font-medium">
        ...
        locale="es"
        // Definir -1 para que comience en el día actual de la semana
        firstDay={-1}
        slotMinTime={minTime}
        slotMaxTime={maxTime}
